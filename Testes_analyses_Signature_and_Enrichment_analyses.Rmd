...
intersect_with_regionCustom = function(vcf, surveyed, region, regionName, triRateTable, tissueType, sample, addSigs = F, sigsAssign = NULL) {
    ...
    if(addSigs) {
      ...
      sigSums <- get_variant_contexts(mutsInRegion, ref_genome) %>% 
        ...
        summarise(across(matches("SBS"), sum))
    ...
    for (j in 1:length(region_list)) {
        for (i in 1:length(vcf_list)) {
            res = intersect_with_regionCustom(vcf_list[[i]], surveyed_list[[i]], 
                region_list[[j]], regionName = names(region_list)[j], triRateTable, tissueType = tissues[i], sample = names(vcf_list[i]), addSigs, sigAssign)
            df = rbind(df, res)
        }
    }
...
if(useSigs) {
  ...
  mean_assignment <- read_tsv(paste0(strandPath, "Decompose_Solution_Activities.txt"), col_types = cols(Samples = "c", .default = "d")) %>% 
    ...
    group_by(header1) %>% 
    summarize(across(matches("SBS"), sum), .groups = "drop") %>% 
  ...
  probabilityDF <- probabilityDF0 %>% 
    left_join(probabilityDF0 %>% group_by(chr, pos, REF, sub, trinuc, sample) %>% summarize(probSum = sum(prob), .groups = "drop"), by = c("chr", "pos", "REF", "sub", "trinuc", "sample")) %>% 
...
#Exlude UTR regions
exons <- setdiff(setdiff(setdiff(exons_hg19, introns_hg19), fiveUTRs_hg19),threeUTRs_hg19)
introns <- setdiff(setdiff(setdiff(introns_hg19, exons_hg19), fiveUTRs_hg19),threeUTRs_hg19)

exonicRegions_list=list()
exonicRegions_list[[1]] <- exons
exonicRegions_list[[2]] <- introns
names(exonicRegions_list) <- c("exonic", "intronic")
...
pRep
pExonic <- codingCIs %>% filter(region != "coding") %>% 
  mutate(by = fct_relevel(by, tissueOrder)) %>% 
  mutate(region = fct_relevel(region, c("intronic", "exonic"))) %>% 
...
module load bedtools
cat PD*.regions.bed | sort -k1,1 -k2,2n | bedtools merge -d -1 -c 4 -o mean > testisCpGmeanCoverage.bed

bsub -o out.%J -e err.%J -q normal -n 4 -R 'select[mem>=4000] rusage[mem=4000]' -M4000 ./mergeCoverageTestis.sh
...
gunzip PD*.regions.bed.gz
module load bedtools
cat PD*.regions.bed | sort -k1,1 -k2,2n | bedtools merge -d -1 -c 4 -o mean > colonCpGmeanCoverage.bed
