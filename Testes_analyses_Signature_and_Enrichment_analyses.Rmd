...
```{r TumourDatasets}
#Pop var 
gnomadVCF <- read_vcfs_as_granges("/Users/mn7/volumes/rs30_lustre/Testicular_project/Final_Analyses/Unmatched_WGS/Signature_Analyses/PopulationVariants/test_gnomad.5.vcf", "gnomad", ref_genome, check_alleles=T)
...
# #Surveyed Regions
# chr5 <- read_tsv("/Users/mn7/volumes/rs30_lustre/Testicular_project/Final_Analyses/Surveyed_regions/Trio_DNMs.callable.bed", col_names = F) %>% 
#    filter(X1 == "5")
# write_tsv(chr5, "/Users/mn7/volumes/rs30_lustre/Testicular_project/Final_Analyses/Surveyed_regions/chr5.bed", col_names = F)
chr5_surveyed <- import("/Users/mn7/volumes/rs30_lustre/Testicular_project/Final_Analyses/Surveyed_regions/chr5.bed")
seqlevelsStyle(chr5_surveyed) <- "UCSC"
full_surveyed <- import("/Users/mn7/volumes/rs30_lustre/Testicular_project/Final_Analyses/Surveyed_regions/Trio_DNMs.callable.bed")
seqlevelsStyle(full_surveyed) <- "UCSC"
load("testesIndivSurveyedList.RData")
load("colonIndivSurveyedList.RData")

condensedVCFs <- c(colonIndivVCF, lungExSmokerVCF, lungNormalVCF, testesIndivVCF, triosMatVCF, triosPatVCF, triosVCF, gnomadVCF)
groupedVCFs <- c(colonIndivVCF[[1]], lungExSmokerVCF[[1]], lungNormalVCF[[1]], testesIndivVCF[[1]], triosMatVCF[[1]], triosPatVCF[[1]], triosVCF[[1]], gnomadVCF[[1]])
condensedTissues <- c(rep("colonIndiv",length(colonIndivSamples)),"LungExSmoker","LungNormal",rep("testesIndiv",length(testesIndivSamples)), "TrioMat_DNMs", "TrioPat_DNMs", "Trio_DNMs", "gnomad")
condensed_surveyed_list <- c(colonIndivSurveyedList, full_surveyed, full_surveyed, testesIndivSurveyedList, full_surveyed, full_surveyed, full_surveyed, chr5_surveyed)
...
meth_dist = genomic_distribution(condensedVCFs, condensed_surveyed_list, meth_regions_list)
meth_depl = enrichment_depletion_test(meth_dist, by = condensedTissues) %>% 
  #Multiple test correction
  mutate(significant = if_else(pval < p_corrected, "*", ""))
...
ggplot(methFraction, aes(x = fraction, y = obsExp)) + 
  geom_point(aes(colour = sample, shape = methylation, size = 2)) + 
  ylab("Obs/Exp") +
  theme_minimal()

ggplot(methFraction, aes(y = fraction, x = obsExp)) + 
  geom_point(aes(colour = sample, shape = methylation, size = 2)) + 
  xlab("Obs/Exp") +
  xlim(0,NA) +
  theme_minimal()
