...
#Testes Tumour
testesTumourVCF <- read_vcfs_as_granges("/Users/mn7/volumes/rs30_lustre/Testicular_project/Final_Analyses/Unmatched_WGS/Signature_Analyses/Tolly_Tumour_data/Tumour_data.vcf", "Testes(Tumour)", ref_genome, check_alleles=T)
...
#Lung Normal
lungVCF <- read_vcfs_as_granges("/Users/mn7/volumes/rs30_lustre/Testicular_project/Final_Analyses/Unmatched_WGS/Signature_Analyses/Kenichi-Lung_data/Lung_data.unique.vcf", "Lung", ref_genome, check_alleles=T)

condensedVCFs <- c(colonVCF, lungVCF, testesVCF, testesBenignVCF, testesTumourVCF, triosVCF)
groupedVCFs <- c(colonVCF[[1]], lungVCF[[1]], testesVCF[[1]], testesBenignVCF[[1]], testesTumourVCF[[1]], triosVCF[[1]])
condensedTissues <- c("Colon","Lung","Testes", "Testes(Benign)", "Testes(Tumour)", "Trio_DNMs")
condensedSource <- tibble(source = c(rep("Colon",length(colonVCF[[1]])), rep("Lung",length(lungVCF[[1]])), rep("Testes",length(testesVCF[[1]])), rep("Testes(Benign)",length(testesBenignVCF[[1]])), rep("Testes(Tumour)",length(testesTumourVCF[[1]])), rep("Trio_DNMs",length(triosVCF[[1]]))))
