...
```{r TumourDatasets}
#Testes Normal 
chr5_1000GPVCF <- read_vcfs_as_granges("/Users/mn7/volumes/rs30_lustre/Testicular_project/Final_Analyses/Unmatched_WGS/Signature_Analyses/PopulationVariants/1000GENOMES-phase_3.chr5.SNVs.recode.vcf", "chr5_1000GP", ref_genome, check_alleles=T)



condensedVCFs <- c(chr5_1000GPVCF)
groupedVCFs <- c(chr5_1000GPVCF[[1]])

condensedTissues <- c("1000GP")
condensedSource <- tibble(source = c(rep("1000GP",length(chr5_1000GPVCF[[1]]))))


#Surveyed Regions: Here just using full genomic coordinates for testing/simplicity. Extend to detailed surveyed regions later.
condensed_surveyed <- import("/Users/mn7/volumes/rs30_lustre/Testicular_project/Final_Analyses/Unmatched_WGS/Signature_Analyses/PopulationVariants/chr5.full.bed")
seqlevelsStyle(condensed_surveyed) <- "UCSC"
condensed_surveyed_list <- rep(list(condensed_surveyed), length(condensedTissues))
```
...
## SigFit Dataset
```{r SigFitData}
repTimingFolder = "/Users/mn7/volumes/rs30_lustre/Testicular_project/Final_Analyses/Unmatched_WGS/Signature_Analyses/ReplicationTiming/"
pyrTri_to_mutOpp <- c(rep(c(1:4,9:12,17:20,25:28), 3), rep(c(5:8,13:16,21:24,29:32),3))

getMutTable <- function(mutFolder) {
  files <- list.files(paste0(repTimingFolder, mutFolder), pattern = ".vcf", full.names = TRUE)
  samples <-  str_remove(list.files(paste0(repTimingFolder, mutFolder), pattern = ".vcf"), ".vcf")
  VCF <- read_vcfs_as_granges(files, samples, ref_genome, check_alleles=T)
  muts <- t(mut_matrix(vcf_list = VCF, ref_genome))
  return(muts)
}

getOppTable <- function(callableFolder) {
  pyrCountFiles <- list.files(paste0(repTimingFolder, callableFolder), pattern = ".pyr.tsv", full.names = TRUE)
  samples <-  str_remove(list.files(paste0(repTimingFolder, callableFolder), pattern = ".pyr.tsv"), ".pyr.tsv")
  opps <- NULL
  for(i in pyrCountFiles) {
    pyrDF <- read_tsv(i, col_names = F, col_types = 'ci')[pyrTri_to_mutOpp,2]
    opps <- bind_cols(opps, pyrDF)
  }
  colnames(opps) <- samples
  opps <- t(opps)
  return(opps)
}

#Testes
testesEarlyMuts <- getMutTable(mutFolder = "Testes_Early/")
testesEarlyOpps <- getOppTable(callableFolder = "SigFit/Testes_Early_Callable/")

testesIntermediateMuts <- getMutTable(mutFolder = "Testes_Intermediate/")
testesIntermediateOpps <- getOppTable(callableFolder = "SigFit/Testes_Intermediate_Callable/")

testesLateMuts <- getMutTable(mutFolder = "Testes_Late/")
testesLateOpps <- getOppTable(callableFolder = "SigFit/Testes_Late_Callable/")

#Colon
colonEarlyMuts <- getMutTable(mutFolder = "Colon_Early/")
colonEarlyOpps <- getOppTable(callableFolder = "SigFit/Colon_Early_Callable/")

colonIntermediateMuts <- getMutTable(mutFolder = "Colon_Intermediate/")
colonIntermediateOpps <- getOppTable(callableFolder = "SigFit/Colon_Intermediate_Callable/")

colonLateMuts <- getMutTable(mutFolder = "Colon_Late/")
colonLateOpps <- getOppTable(callableFolder = "SigFit/Colon_Late_Callable/")
...
#Choose 1, 5 and 18 for testes and colon
selected_signatures <- cosmic_signatures_v3[rownames(cosmic_signatures_v3) %in% c("SBS1", "SBS5", "SBS18"),]
#sigfit::plot_spectrum(testesEarlyMuts)

plotExposureSimple <- function(muts, opps = NULL){
  ...
  exposurePlot <- tibble(sample = row.names(exposures), exposures[1:3]) %>% 
    pivot_longer(-sample, names_to = "signature") %>% 
    mutate(signature = fct_relevel(signature, c("SBS18", "SBS5","SBS1"))) %>% 
...
ptestesEarlyN <- plotExposureSimple(muts = testesEarlyMuts, opps = NULL)
ptestesEarlyG <- plotExposureSimple(muts = testesEarlyMuts, opps = "human-genome")
ptestesEarly <- plotExposureSimple(muts = testesEarlyMuts, opps = testesEarlyOpps)
ptestesEarlyN

ptestesEarly

ptestesIntermediateN <- plotExposureSimple(muts = testesIntermediateMuts, opps = NULL)
ptestesIntermediateG <- plotExposureSimple(muts = testesIntermediateMuts, opps = "human-genome")
ptestesIntermediate <- plotExposureSimple(muts = testesIntermediateMuts, opps = testesIntermediateOpps)
ptestesLateN <- plotExposureSimple(muts = testesLateMuts, opps = NULL)
ptestesLateG <- plotExposureSimple(muts = testesLateMuts, opps = "human-genome")
ptestesLate <- plotExposureSimple(muts = testesLateMuts, opps = testesLateOpps)

pcolonEarlyN <- plotExposureSimple(muts = colonEarlyMuts, opps = NULL)
pcolonEarly <- plotExposureSimple(muts = colonEarlyMuts, opps = colonEarlyOpps)
pcolonIntermediateN <- plotExposureSimple(muts = colonIntermediateMuts, opps = NULL)
pcolonIntermediate <- plotExposureSimple(muts = colonIntermediateMuts, opps = colonIntermediateOpps)
pcolonLateN <- plotExposureSimple(muts = colonLateMuts, opps = NULL)
pcolonLate <- plotExposureSimple(muts = colonLateMuts, opps = colonLateOpps)

ptestesEarlyN
ptestesEarlyG
ptestesEarly
ptestesIntermediateN
ptestesIntermediateG
ptestesIntermediate
ptestesLateN
ptestesLateG
ptestesLate
