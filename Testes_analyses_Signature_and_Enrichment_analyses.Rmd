...
tissuesByVar <- c(rep("colon",length(unlist(colonVCF))), rep("pop_vars", length(unlist(popVarsVCF))), rep("testis",length(unlist(testesVCF))), rep("trio_DNMs", length(unlist(triosVCF))))
...
tissuesByVar <- c(rep("colon",length(unlist(colonVCF))), rep("testis",length(unlist(testesVCF))), rep("trio_DNMs", length(unlist(triosVCF))))
...
tissuesByVar <- c(rep("testis_seminiferous_tubule",length(unlist(testesVCF))), rep("colon_crypt",length(unlist(colonVCF))), rep(panBodyTissues3, times = lengths(panBody3VCF)))
type_occurrences <- mut_type_occurrences(VCFs, ref_genome)
#Surveyed Regions
testesSurveyed <- load_callable("Testes")
colonSurveyed <- load_callable("Colon")
...
#tissuesWithExprData <- c("testis","colon","trio_DNMs", "pop_vars")
tissuesWithExprData <- c("adrenal_gland","liver")
