...
# TriNuc Adjustment
## TriNuc Counter script
```{r trinucCounter, eval = FALSE}
library(Biostrings)
library(seqinr)
...
unmasked_genome_file = NULL
genome_file          = args[1]     # "/nfs/cancer_ref01/Homo_sapiens/37/genome.fa"
out_name             = args[2]     # "human_strict"
if(length(args)==3) {
	unmasked_genome_file = args[3] # "/lustre/scratch119/casm/team268im/fa8/BOTSEQ/MASKS/all-norm-strict.inverse.bed"
}
###########################################################################################
# Load sequences, either for regions or the entire genome
if(!is.null(unmasked_genome_file)) {
	cat("Loading regions...\n")
	regions = read.table(unmasked_genome_file,header=F,sep="\t",colClasses=c("character","numeric","numeric"))
	colnames(regions) <- c("chr","start","end")
	regions$start = regions$start+1 # make it 1-based
	
	cat("Extracting sequences...\n")
	sequences = as.vector(scanFa(genome_file, GRanges(regions$chr, IRanges(regions$start, regions$end))))
} else {
	cat("Loading entire genome...\n")
	sequences = as.vector(scanFa(genome_file))
}
	
###########################################################################################
# trinucleotideFrequency
cat("Calculating trinucleotide frequencies...\n")
tri_freqs <- t(sapply(sequences,function(x) trinucleotideFrequency(DNAString(x))))

tri_freqs_summary <- apply(tri_freqs,2,sum)
write.table(tri_freqs_summary,file=paste(out_name,".tri_freqs.tsv",sep=""),sep="\t",quote=F,col.names=F)

###########################################################################################
# Translate to pyr reference:
complement = vector()
complement["A"] = "T"; complement["C"] = "G"; complement["G"] = "C"; complement["T"] = "A";		
# Define pyr_trinucs
pyr = c("C","T")
...
for(a in all) {
	for(p in pyr) {
		for(b in all) {
			pyr2pur[paste(a,p,b,sep="")] = paste(complement[b],complement[p],complement[a],sep="")
		}
	}
}
tri_freqs_summary_pyr = vector()
for(tri_pyr in names(pyr2pur)) {
	tri_freqs_summary_pyr[tri_pyr] = tri_freqs_summary[tri_pyr] + tri_freqs_summary[pyr2pur[tri_pyr]]
}
write.table(tri_freqs_summary_pyr,file=paste(out_name,".tri_freqs.pyr.tsv",sep=""),sep="\t",quote=F,col.names=F)


```

## TriNuc Bash scripts
```{bash trinucBash, eval = FALSE}
#Testis Samples
#!/bin/bash

GENOME=/nfs/cancer_ref01/Homo_sapiens/37/genome.fa
CALLABLE=/lustre/scratch119/casm/team294rr/rs30/Testicular_project/mosdepth/WGS/Callable_regions_for_CAVEMAN/Per_PID-Tissue/Normal
INDIVs=(PD40744c PD40745c_PD42564c PD40746c_PD42568c PD42034b PD42036b PD42563c PD42565c PD42566c PD42569c PD43726c PD43727b PD46269c)
OUT=/lustre/scratch119/casm/team294rr/mn7/pipelineInput/triNucBackground/Testes

#Get trinucleotide counts for callable regions
for indiv in ${INDIVs[@]}
do
	Rscript triNucCounter.R $GENOME $OUT/${indiv}.full $CALLABLE/${indiv}.sorted.merged.bed
done	
	
#Commands
bsub -o out.%J -e err.%J -q normal -n 4 -R 'select[mem>=4000] rusage[mem=4000]' -M4000 ./testesTriNuc.sh

#Colon Samples
#!/bin/bash

GENOME=/nfs/cancer_ref01/Homo_sapiens/37/genome.fa
CALLABLE=/lustre/scratch119/casm/team294rr/rs30/Testicular_project/mosdepth/WGS/Callable_regions_for_CAVEMAN/Per_PID-Tissue/Colon
INDIVs=(PD40745b_PD42564b PD40746b_PD42568b PD42563b PD42566b)
OUT=/lustre/scratch119/casm/team294rr/mn7/pipelineInput/triNucBackground/Colon
...
#Commands
bsub -o out.%J -e err.%J -q normal -n 4 -R 'select[mem>=4000] rusage[mem=4000]' -M4000 ./colonTriNuc.sh
```

## Background Rate
```{r trinucBackground, fig.height=4, fig.width=10}
pyrTri_to_mutOpp <- c(rep(c(1:4,9:12,17:20,25:28), 3), rep(c(5:8,13:16,21:24,29:32),3))

getRateTable <- function(vcf_list, mutFolder, suffix = ".tri_freqs.pyr.tsv") {
  #Get total muts for tissue
  muts0 <- mut_matrix(vcf_list, ref_genome)
  muts <- as_tibble(muts0) %>% 
    mutate(mutType = rownames(muts0)) %>% 
    rowwise() %>%
    mutate(totMuts = sum(c_across(where(is.numeric))))
  #Get mean opportunities for tissue
  pyrCountFiles <- list.files(mutFolder, pattern = suffix, full.names = TRUE)
  samples <- str_remove(list.files(mutFolder, pattern = suffix), suffix)
  opps <- NULL
  for(i in 1:length(pyrCountFiles)) {
    pyrDF <- read_tsv(pyrCountFiles[i], col_names = F, col_types = 'ci')[pyrTri_to_mutOpp,2] 
    colnames(pyrDF) <- samples[i]
    opps <- bind_cols(opps, pyrDF)
  }
  opp_to <- opps %>% 
    rowwise() %>%
    mutate(meanOpps = mean(c_across(cols = everything())))
  freqDF <- bind_cols(muts, opp_to) %>% 
    select(mutType, totMuts, meanOpps) %>% 
    mutate(rate = totMuts/meanOpps) %>% 
    mutate(context = paste0(str_sub(mutType,1,1),str_sub(mutType,3,3),str_sub(mutType,7,7))) %>% 
    group_by(context) %>% 
    summarise(rate = sum(rate), .groups = "drop") %>% 
    mutate(rate = rate/sum(lengths(vcf_list)))
  # scaledDF <- freqDF %>% 
  #   mutate(scaledRate = rate * (1/sum(freqDF$rate))) %>% 
  #   select(-rate)
  return(freqDF)
}

colonTriRate <- getRateTable(colonVCF, mutFolder = paste0(path, "triNucBackground/Colon"), suffix = ".tri_freqs.pyr.tsv")
testesTriRate <- getRateTable(testesVCF, mutFolder = paste0(path, "triNucBackground/Testes"), suffix = ".tri_freqs.pyr.tsv")

colonTriRate %>% 
  ggplot(aes(x = context, y = scaledRate)) +
  geom_point() + 
  theme_minimal() +
...
testesRepDist <- genomic_distributionCustom(testesVCF, testesSurveyed, regions_list, testesTriRate)
yo2 = testesRepDist %>% 
  group_by(region) %>% 
  summarise(across(where(is.numeric),sum), .groups = "drop") %>% 
  mutate(by = "Testis") %>% 
...
plot_enrichment_depletion_prop(yo2)
...
strand_bias <- strand_bias_test(strand_counts) %>% 
  ...
  mutate(class = if_else(str_detect(group, "Testis"), "Germline", "Soma")) %>% 
  mutate(class = fct_relevel(class, "Soma", "Germline"))
...
txBiasSummary <- ggplot(strand_bias, aes(type, logRatio, color = group, label = significant)) +
  ...
  geom_jitter(position = position_jitter(width = 0.2, seed = 1), size = 3, aes(shape = class)) + 
  theme_bw() +
